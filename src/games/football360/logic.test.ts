...
describe('football360 logic (reciprocal turn-taking, identical to rollback)', () => {
  it('buildPlayers puts the child first and fills teammates', () => {
  ...
  it('every rally targets a teammate and lists the rest as distractors', () => {
  ...
  it('the ready teammate never repeats back-to-back when there is a choice', () => {
    const cfg = CONFIG.hard
  ...
  it('easy mode degenerates to a clean dyad (single teammate, no traps)', () => {
  ...
  it('classifyReturn separates premature, wrong-partner, and correct', () => {
    ...
    // any pass before the ready cue is premature, even to the right teammate
...
describe('football360 pitch layout (360 view)', () => {
  it('the child stands on the centre spot', () => {
    expect(playerPosition(0, 3)).toEqual([0, 0])
  })

  it('every teammate stands one radius away, in front of the child', () => {
    for (const partners of [1, 2, 3] as const) {
      for (let i = 1; i <= partners; i++) {
        const [x, z] = playerPosition(i, partners)
        expect(Math.hypot(x, z)).toBeCloseTo(PARTNER_RADIUS, 5)
        expect(z).toBeLessThan(0) // never behind the child
      }
    }
  })

  it('bearings match the arc table and spread wider with more teammates', () => {
    expect(playerHeadingDeg(1, 1)).toBe(0)
    expect(playerHeadingDeg(1, 2)).toBe(PARTNER_BEARINGS[2][0])
    expect(playerHeadingDeg(2, 2)).toBe(PARTNER_BEARINGS[2][1])
    expect(playerHeadingDeg(1, 3)).toBe(-50)
    expect(playerHeadingDeg(2, 3)).toBe(0)
    expect(playerHeadingDeg(3, 3)).toBe(50)
  })

  it('a tap right after a look-around drag is swallowed; a clean tap is not', () => {
    lookDrag.px = 40
    lookDrag.endedAt = 1000
    expect(isDragTail(1100)).toBe(true) // just finished dragging
    expect(isDragTail(2000)).toBe(false) // long after the drag
    lookDrag.px = 3
    expect(isDragTail(1100)).toBe(false) // too small to be a drag
  })
})
